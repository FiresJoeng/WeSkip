...
namespace Speedhack
{
    ...
    void Detach()
    {
        if (!initialised) return; // 防止重复反初始化
...
// speedhack.dll 注入接口
extern "C" __declspec(dllexport) void InjectSpeedhack(double speed)
{
    Speedhack::Setup();
...
// speedhack.dll 卸载接口
extern "C" __declspec(dllexport) void EjectSpeedhack()
{
    Speedhack::Detach();
}